...
of **three backends** — `python3` (standard library only, macOS/Linux) and
...
at a hidden prompt, and nothing else ever reads or stores it. (The Codex and
Grok backends need no key; `init` offers each when a usable CLI is detected.)
...
Cost depends on the backend. On the **Codex** and **Grok** backends there is
**no per-image charge** — generation runs on your Codex or Grok subscription and
draws on that quota (image turns consume it faster than text turns), and the
image model is automatic (no model selection). On the **OpenRouter** backend
