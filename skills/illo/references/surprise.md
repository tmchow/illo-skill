...
2. **Step 0 preflight** (`doctor`) — resolve hard blockers (including
   ...
   saying work or picker. On Grok Bot native, the backend-choice/no-key result
   is informational as described in `SKILL.md`; still fix asset, pack, palette,
   or path failures. Use the installed-character list from this check for
   character resolution below.
